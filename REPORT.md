...
Эти числа — результат пересчёта после исправления метрики, описанного в §9.6. В первой версии этого
раздела стояли значения, посчитанные **до** исправления (0.528 / 0.469 / 0.461 / 0.417 / 0.380 / 0.202), и
они остались в отчёте по механической причине: `pareto.py` писал артефакты по фиксированным именам, поэтому
пересчёт раунда 2 затёр файлы раунда 1, и таблица раунда 1 не обновилась вместе с кодом. Дефект устранён —
имена артефактов теперь несут префикс раунда (`r1_`, `r2_`), так что затирание невозможно. Качественные
выводы не изменились: ни одно плечо не превосходит наивный стиринг, единственные значимые эффекты
отрицательные, MTS худшее. Изменились две детали: у CDS и сохранения нормы интервалы теперь **не содержат
нуля**, то есть их проигрыш стал значимым, а FSR сдвинулся ещё ближе к нулю.
...
Из §7.5 видно, что у наивного сложения `h + s·v̂` наименьшее нелинейное повреждение `C` среди всех
проверенных плеч, и каждая попытка ремонта его увеличивает. Первая версия этого раздела делала отсюда
...
использует все 96 ячеек (12 признаков × 8 сил). Это объясняет разницу в мощности, но не отменяет того, что
