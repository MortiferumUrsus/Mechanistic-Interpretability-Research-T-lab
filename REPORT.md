...
Из §7.5 следует, что ремонт после интервенции работать не может: у наивного сложения `h + s·v̂` уже
наименьшее нелинейное повреждение среди всех проверенных плеч, и каждая попытка ремонта его
увеличивает. Отдельной поломки, лежащей ортогонально полезному сигналу, в отклике нет. Значит
единственный оставшийся рычаг — не то, что делать после интервенции, а **что именно инжектировать**.
...
целевого признака SAE выше в 5–6 раз. Результат DEV воспроизвёлся на признаках, которых поправка не
видела.

**Статистика.** У этого дизайна плечи согласованы по построению: при одном `c` они инжектируют
возмущение одной нормы. Поэтому основной тест — парный по ячейкам (признак, сила), bootstrap по
признакам:
...
Первичный endpoint при бюджете наивного стиринга при `c = 1` даёт +0.072 с интервалом
[−0.141, +0.271]; при бюджетах `c = 1.5` и `c = 2.0` — +0.072 и +0.127. Endpoint сжимает два фронта в
одно число при одном бюджете и теряет мощность по сравнению с парным тестом при равной силе, поэтому
основным здесь объявлен парный тест, а endpoint приведён как сопоставимая с первым раундом величина.
...
исправление сформулировано симметрично и применено ко всем плечам.
...
### 9.8 Контроль специфичности на TEST
...
Доля попаданий по словарям чужих признаков у поправки падает с ростом силы (0.078 → 0.044), то есть
текст не становится «вообще выразительным». Специфичность у поправки выше наивного стиринга на всех
силах и не падает к `c = 3`, тогда как у наивного падает вдвое.
...
Более того, у наивного сложения `h + s·v̂` уже наименьшее нелинейное повреждение среди всех проверенных
плеч, то есть отдельной поломки, подлежащей удалению, в отклике нет. Три естественные починки — убрать
