...
Утверждение здесь надо сформулировать точно, потому что в первой версии оно было сформулировано сильнее,
чем позволяют данные. Я писал, что у наивной интервенции наименьшее `C` среди всех плеч на всех силах.
Это неверно: у сохранения нормы `C` **ниже** наивного на четырёх силах из пяти (0.149 против 0.164 при
`c = 0.5`; 0.488 против 0.533 при `c = 1`, то есть в выделенной строке выше; 1.524 против 1.583 при
`c = 2`; 2.802 против 3.028 при `c = 3`), и по ячейкам (признак, сила) минимум приходится на него в 45
случаях из 60 против 15 у наивного.

Что действительно следует из таблицы и держится на всех силах: **любая постобработка активации увеличивает
...
`c = 1` и выше**; при `c = 0.5` они на одном уровне и наивная даже номинально впереди (0.138 против
0.120) — строку с самой слабой силой я в первой версии таблицы опустил, а опускать её не следовало, потому
что она и есть граница, где преимущество появляется. К `c = 3` специфичность наивного падает с 0.354 до
0.225, то есть в 1.6 раза (в первой версии было сказано «вдвое» — это преувеличение), а у поправки
держится на 0.475.
