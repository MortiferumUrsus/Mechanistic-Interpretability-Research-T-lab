...
этом лучше на 0.77…1.41 ната log-PPL (парная разность по силам `c ≥ 1`, `results/cells.csv`). Значит `C`
не монотонно связан с текстовым повреждением, и
...
не растёт (§7.5, таблица). У поправки направления `C` растёт на 2–10%, а `A` — на 15–31% (§9.3), поэтому
она выигрывает, несмотря на больший `C`. Порядок величин здесь скромный по обеим осям; в разы растёт не
`A`, а концепт-метрика downstream (§9.4), и это разные вещи. Отсюда и мотивировка: рычаг не в том, чтобы уменьшать `C` после
...
Замер на отложенных DEV-направлениях, усреднение по признакам и промптам. Числа ниже — строки
`checkpoint = dir_hot` в [`results/dirfix_eval_dev.csv`](results/dirfix_eval_dev.csv); в том же файле
лежит `dir_smoke`, почти-тождественная модель, которая держалась как проверка на то, что пайплайн замера
не создаёт выигрыш сам по себе. Насколько она тождественна, надо сказать точно: косинус с исходным
направлением 0.9995…0.9999, но отношения не «все в пределах 1%», как утверждала предыдущая версия этого
абзаца, — `A/A_naive` лежит в 0.997…1.034, `C/C_naive` в 0.943…1.036, и за пределы одного процента
выходят 11 из 30 строк по `A` и 12 из 30 по `C`. То есть шум пайплайна замера на отдельной ячейке
доходит до 3–6%, и различать им эффекты меньше этого порядка нельзя; эффект самой поправки (ниже) на
рабочих силах в 5–10 раз больше. Усреднять два чекпойнта вместе нельзя — получится смесь метода с
тождеством, — поэтому чекпойнт назван прямо здесь:
...
**Оговорка о версии чекпойнта, общая для §9.3–§9.5 и §9.7.** Все DEV-замеры сделаны чекпойнтом до
переобучения, то есть до исключения двенадцати признаков раунда 3 из обучающего пула (§9.2), и после
переобучения не пересчитывались: `results/dirfix_eval_dev.csv` и `results/control_specificity_dev.csv`
относятся к прежней версии `dir_hot`, а DEV-генерации перенесены в `results/stale/`. Числа TEST и раунда 3
(§9.6, §9.8–§9.10) посчитаны текущим чекпойнтом. Практическое следствие для косинуса: 0.866 — это среднее
по шести DEV-направлениям для прежней версии; у текущей по двенадцати TEST-направлениям среднее 0.872
(§9.8), то есть величина не изменилась, но проверена она на другом наборе.

### 9.4 Текстовый уровень на DEV

Источник таблицы — `results/stale/scored_dev_r2.csv`: после перевыпуска генераций (§9.6) DEV-прогон не
повторялся, и его файлы лежат в `results/stale/`.
...
_Заранее: этот DEV-вариант контроля дефектен и заменён контролем §9.10а. Дефект — в том, как строилась
случайная ось: здесь она бралась с одним фиксированным сидом на все признаки, то есть шесть «независимых»
контролей были поворотами одной и той же гауссовой выборки, и разброс между признаками занижен. В §9.10а
ось выводится из самого направления (контрольная сумма направления как сид), так что воспроизводимость
сохраняется, а выборки независимы; этот прогон с исправленным контролем сделан заново на свежих
признаках. DEV-таблица ниже оставлена как есть, потому что на неё опирался выбор метода, но выводы из неё
не делаются._
...
На DEV случайный поворот лежит на фронте наивного стиринга: перплексия совпадает в пределах 0.17 ната,
концепт — в пределах шума. Вывод отсюда — тот же, что и в §9.10а на свежих признаках, но опирается он
именно на §9.10а: здесь и признаки те, на которых метод выбирался, и случайная ось общая для всех шести.

Две оговорки, обе про границы этого контроля. Первая: он посчитан **только на DEV** и общей случайной
осью; версия на настоящем тестовом сплите с независимыми осями — в §9.10а. Вторая: `cos = 0.866` — это средний угол.
Поправка низкоранговая (`w(v) = normalise(v + ABᵀv)`), поэтому фактический угол зависит от признака: по шести
DEV-признакам это 0.721, 0.827, 0.861, 0.912, 0.915, 0.959 — среднее и даёт 0.866, но крайние значения
отличаются от него на 0.15 и 0.09. Случайный поворот сматчен со средним, а не с каждым по отдельности. Для утверждения «дело не в угле как
...
Тот же раунд прогнал контроль случайного поворота — впервые на настоящем тестовом сплите (§9.7 считался
только на DEV) и впервые с независимыми осями: ось выводится из контрольной суммы самого направления, а не
из одного сида на все признаки, поэтому двенадцать контролей независимы, а прогон воспроизводим. Угол тот
же, норма та же.
...
меняется на противоположный, и это тот случай, где ошибка была не в интерпретации, а в данных.
...
обучающий сид один и разброс от переобучения не измерен (§4), TEST открывался дважды (§9.6), DEV-замеры
раунда 2 сделаны чекпойнтом до переобучения (§9.3).
