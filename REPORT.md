...
значение `s`, при котором упрощение совпадёт с замером (получалось `s ≈ 8`, ниже всего наблюдаемого
диапазона 11.3…19.6), и делала из этого вывод, что линейно-гауссовская оценка недооценивает стирание. Вывод
...
`checkpoint = dir_hot` в `results/dirfix_eval_dev.csv`; в том же файле лежит `dir_smoke`, почти-тождественная
модель (`cos = 1.000`, все отношения в пределах 1%), которая держалась как проверка на то, что пайплайн
замера не создаёт выигрыш сам по себе. Усреднять два чекпойнта вместе нельзя — получится смесь метода с
...
Полноценная проверка требует свежего набора признаков — она **сделана** и описана в §9.10, и её исход
частичный: текстовая половина выигрыша воспроизвелась значимо, концептная на двенадцати признаках не
установлена.
...
_Заранее: вывод этого раздела уточнён в §9.10а по результатам на свежих признаках. Кратко — случайный
поворот не воспроизводит совместный выигрыш по обеим осям, но концептную ось он воспроизводит, и это видно
уже в таблице ниже, если читать её по столбцам, а не по совокупности._
...
признаков, на который не смотрел никто. Двенадцать таких отобраны тем же объективным правилом
(`src/select_round3.py`), с проверкой, что они не пересекаются с замороженными сплитами и не коррелируют с
ними (`|cos| < 0.3`), и добавлены как `test_r3`, не трогая `test` и `dev`. Отбор закоммичен до генерации.
...
|---|---|---|
| primary endpoint, наивный | 0.534 | 0.538 |
| primary endpoint, поправка | 0.527 | **0.670** |
| разность, 95% бутстрап | — | **+0.139** [−0.118, +0.370], `p(Δ>0) = 0.87` |
| log-PPL, парная разность | — | **−1.75** [−2.49, −1.06] |
| концепт, парная разность | — | +0.205 [−0.009, +0.429] |
| `p(доминирует по обеим осям)` | — | **0.97** |

**Что воспроизвелось, а что нет.** Направление воспроизвелось по обеим осям, но установлены они по-разному.
Выигрыш по **тексту** значим: парная разность log-PPL равна −1.75, и интервал ноль не накрывает. Выигрыш по
**концепту** — +0.205 с интервалом [−0.009, +0.429], то есть на грани; на двенадцати признаках он не
установлен. Совместное доминирование по обеим осям имеет вероятность 0.97. Точечная оценка endpoint выросла
(0.670 против 0.538), но её интервал ноль накрывает, и это ожидаемо: endpoint — величина с большой
межпризнаковой дисперсией, а признаков двенадцать.

Честная формулировка результата: **на свежих признаках подтверждается, что поправка направления снижает
текстовое повреждение, и не подтверждается — на этой выборке — что она повышает концепт-метрику.** Это
слабее, чем раунд 2 на TEST, и слабее ровно в том месте, где ослабление и ожидается при переходе от
признаков, на которых метод выбирался, к признакам, на которых он не выбирался.

**Контроль специфичности воспроизводится и он резче основного результата.** Специфичность поправки держится
на 0.45…0.50 при силах от 1 и выше, тогда как у наивного стиринга она достигает 0.44 при `c = 1` и падает до
0.19 к `c = 3`. То есть свойство, ради которого метод и строился — сохранять признак-специфичность при
росте силы, — на свежих признаках видно так же ясно, как на TEST.

### 9.10а Случайный поворот на свежих признаках: он забирает половину результата, и это меняет вывод

Тот же раунд прогнал и контроль случайного поворота, которого раньше не было ни на одном настоящем тестовом
сплите (§9.7 считался только на DEV). На свежих признаках он ведёт себя иначе, чем я утверждал, и это надо
записать прямо.

| `c` | наивный log-PPL / концепт | случайный поворот | поправка |
|---|---|---|---|
| 1.0 | 5.11 / 0.531 | 5.03 / **0.581** | **4.14** / 0.575 |
| 1.5 | 6.02 / 0.383 | 6.20 / **0.556** | **4.57** / 0.611 |
| 2.0 | 6.20 / 0.286 | 6.66 / **0.531** | **4.22** / 0.564 |
| 3.0 | 6.08 / 0.242 | 6.73 / **0.442** | **3.41** / 0.514 |

Случайный поворот **воспроизводит концептную половину выигрыша почти целиком**: 0.581 / 0.556 / 0.531 / 0.442
против 0.575 / 0.611 / 0.564 / 0.514 у обученной поправки, тогда как наивный стиринг обваливается с 0.531 до
0.242. И он **не воспроизводит текстовую половину вообще**: его log-PPL 5.03…6.73 не лучше наивного, а
поправка идёт по 4.14…3.41.

Отсюда содержательный вывод, которого у меня до раунда 3 не было: **уход со столбца декодера — это то, что
покупает выражение концепта на больших силах, и покупает его любой поворот. Обученная поправка добавляет не
это, а способность сделать то же самое, не разрушая текст.** Вклад обучения лежит на текстовой оси.

Это согласуется с §9.10 само по себе: там значимой оказалась именно текстовая половина разности (−1.75,
интервал ноль не накрывает), а концептная — нет (+0.205, интервал ноль накрывает). Два независимых среза
данных говорят одно и то же.

И это частично **отменяет** формулировку §9.7. Там сказано, что случайный поворот выигрыш не воспроизводит.
Для совместного выигрыша по обеим осям это верно и на DEV, и здесь. Для концептной оси в отдельности — нет,
и в DEV-таблице это было видно (концепт 0.483 у поворота против 0.350 у наивного при `c = 2`), но я прочитал
её как «контроль не работает» вместо «контроль забирает одну ось из двух». Раунд 3 сделал это очевидным,
потому что на свежих признаках разрыв по концепту между поворотом и поправкой исчезает совсем.

**Контроль равной концепт-координаты тоже воспроизводится.** При интерполяции наивного стиринга к той же
проекции на столбец декодера разность log-PPL остаётся −1.54 [−2.17, −0.93], то есть механическое
объяснение («поправка просто вкладывает больше по нужной координате») текстовый выигрыш не объясняет.
...
**Третий раунд на признаках, которых работа не видела, уточнил, из чего этот выигрыш состоит, и уточнение
важнее самого воспроизведения.** Уход со столбца декодера покупает выражение концепта на больших силах —
и покупает его **любой** поворот на тот же угол, включая случайный: на свежих признаках контроль даёт
концепт 0.58/0.56/0.53/0.44 против 0.58/0.61/0.56/0.51 у обученной поправки. Чего случайный поворот не даёт
совсем — это сохранности текста: его log-PPL не лучше наивного (5.03…6.73), тогда как у поправки 4.14…3.41.
Значит вклад **обучения** лежит на текстовой оси, а не на концептной, и это подтверждается независимо: в
парном сравнении на свежих признаках значима именно текстовая разность (−1.75, интервал не накрывает ноль),
а концептная — нет (+0.205, накрывает).

Формулировка, которую эта работа готова защищать: обученная поправка направления — это способ уйти со
столбца декодера SAE **не ломая текст**, а не способ уйти с него вообще.
