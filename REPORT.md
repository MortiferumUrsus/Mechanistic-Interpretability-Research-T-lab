...
Дисперсия вдоль направлений — не одно число: по TEST-признакам она лежит в диапазоне 10.4…23.2 (медиана
13.9), по DEV — 11.3…19.6 (медиана 15.7). Значение `s ≈ 8`, при котором формула даёт `τ ≈ 0.44`, лежит
**ниже всего наблюдаемого диапазона**, а не в его нижней части: при фактическом минимуме 10.4 замкнутая
форма даёт `τ = 0.51`, при медиане TEST — `0.58`, то есть заметно выше измеренных `0.438`. Честная
формулировка: замкнутая форма даёт правильный порядок величины и правильную зависимость от `σ` и от
положения в спектре, но количественно **не воспроизводит** `0.438` ни при одном значении `s` из
измеренного диапазона — линейно-гауссовская оценка систематически недооценивает стирание. Проверкой без свободных
