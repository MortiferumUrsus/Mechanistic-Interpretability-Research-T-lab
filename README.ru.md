...
3. **Её выигрыш по перплексии на сгенерированном тексте в основном артефакт.** Половина поправки, в
   среднем по признакам, — одно общее направление, которое снижает энтропию следующего токена
   модели. Инжектированное отдельно, оно снижает Pythia log-PPL сильнее, чем полная поправка, при
   силах до `c = 2`, не доставляя никакого концепта, и при этом ухудшает предсказание настоящего
   текста моделью; приставленное к столбцу декодера вообще
...
4. **На настоящем тексте при равной доставке концепта обученная поправка всё же стоит дешевле
   ...
   величину. Вариант со штрафом на энтропию уменьшает среднюю общую компоненту и сохраняет знак
   этого сравнения (§10.6, §10.7).
