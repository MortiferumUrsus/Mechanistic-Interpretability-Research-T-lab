...
## Артефакт: обязательный шаг сдачи, который нужно выполнить руками

Условие задания требует прямо: «сохраните лучший адаптер или чекпойнт в открытый репозиторий на
Huggingface». Это **не** опциональный шаг, и без него сдача неполна.

В [`artifacts/`](artifacts/) лежит всё готовое: `direction_correction.pt` (поправка направления, метод
раунда 2 — именно её стоит публиковать как лучший артефакт), `denoiser.pt`, `config.json` с
