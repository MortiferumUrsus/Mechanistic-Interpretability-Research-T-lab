...
Полный конвейер первого раунда: `scripts\run_all.ps1`. Одна оговорка к нему: он вызывает
`train_denoiser.py` без `--steps`, а по умолчанию там 12000 шагов, тогда как замороженный денойзер
обучен на 60000 (`results/train_mlp_gauss_cond1_s0.json`). Воспроизводить нужно постадийными командами
ниже, где бюджет указан явно, иначе получится другой чекпойнт.
...
`scripts\run_round3.ps1` покрывает шаги 1, 3 и 4; шаг 2 в нём отсутствует и выполняется отдельно.
Артефакты недействительной первой версии третьего раунда лежат в `results/stale/` и в выводах не
участвуют.
